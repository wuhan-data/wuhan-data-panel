...
<body>
			 ...
			 <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-header">
                             栏目管理 
                             <div><small><a href="#" onclick="backFirstMenu('${type_name}')">${type_name}(${label_name})</a> > </small></div>
                        </h1>
                    </div>
                </div> 
                 <!-- /. ROW  -->
   ...
   <div class="btns col-md-6" id="addElement">
      ...
      <button class="btn btn-primary" id="reset" onclick="dosaveSeq('updateThemeWeight','${label_id}')"><i class="fa fa-cog"></i>设置</button>
    </div>  
    ...
    <div class="clear"></div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
     <c:forEach items="${indianalysisSecondByPage}" var="c" varStatus="st">
        <tr id="${c.theme_weight }">
            <td>${type_name}</td>
            <td>${label_name}</td>
            <td>${c.theme_name}</td>
            <td>${c.theme_weight}</td>
<!--             <td>1300048</td>
            <td>工业生产排放量</td> -->
            <td width="45%">
...
<i class="fa fa-edit"></i>修改
 ...
 --%>
 <a href="#" onclick="showPlateClick('${c.theme_id }','${label_id }','${type_name}','${label_name }','colPlateInit')">
<div class="btn btn-success btn-sm"><i class="fa fa-search"></i>查看板块
</div>
</a>
<div class="btn btn-warning btn-sm" style="margin-right:3px" data-toggle="modal" data-target="#myEditModal" onclick="edit('${c.theme_name}',${c.theme_id})">
<i class="fa fa-edit"></i>修改
</div>
<a href="#" onclick="delClick('${c.theme_id }','delTheme')">
...
<div class="modal fade" id="myAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
     ...
     二级栏目：<input class="form-control" type="search" placeholder="请输入二级栏目名称" name="addThemeName">
   ...
   <script src="assets/js/dataTables/jquery.dataTables.js"></script>
    ...
    <script>
    		...
    		dosaveSeq = function(Url,label_id){
              ...
              addClick = function(Url) {
                ...
                delClick = function(id,Url) {
                   ...
                   updateShowClick = function(is_show,id,Url) {
                     $.ajax({
                                type: 'GET',
                                url:  Url+"?theme_id="+id+"&is_show="+is_show,
                                dataType: "html",
                           	    async : false,
                            	  contentType: false, //不设置内容类型
                           	    processData: false,
                       ...
                       pageGoClick = function(Url) {
                      	var currentPage = document.getElementById("currentPageText").value;
                           $.ajax({
                            ...
                            showPlateClick = function(id,label_id,typeName,labelName,Url) { 
                            	var type=encodeURI(encodeURI(typeName));
                            	var label=encodeURI(encodeURI(labelName));
                                 $.ajax({
                                            type: 'GET',
                                            url:  Url+"?theme_id="+id+"&type_name="+type+"&label_name="+label+"&label_id="+label_id,
                                            dataType: "html",
                                       	    async : false,
                                        	contentType: false, //不设置内容类型
                                       	    processData: false,

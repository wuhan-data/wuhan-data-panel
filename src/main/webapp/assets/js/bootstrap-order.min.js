    ...
    var f = {
        ...
        menu: {
            collapse: function(k, j, i) {
                var l = !!k.data("order-menu-expanse")
                  , h = d("[order-type='list']", k)
                  , g = d("[order-type='list-title']", k);
                if (!l && !k.is(":animated")) {
                    k.animate({
                        "width": 140
                    }, 500, function() {
            ...
            expanse: function(l, j, i) {
                var k = !l.data("order-menu-expanse")
                  , h = d("[order-type='list']", l)
                  , g = d("[order-type='list-title']", l);
                if (!k && !l.is(":animated")) {
                    l.animate({
                        "width": 320
                    }, 500, function() {
            ...
            showOperation: function() {
                var h = d(document)
                  , g = h.data("order-menu-operation") || (function() {
        ...
        bindEvt: function(j) {
            var g = d("[order-type='menu']", j)
              , k = d("[order-type='menu-expanse']", g)
              , o = d("[order-type='menu-collapse']", g)
              , p = d("[order-type='menu-operation']", g)
              , r = d("[order-type='list']", j)
              , s = d(".list-group-item", r)
              , h = d("[order-type='num']", r)
              , l = (function() {
            ...
            var n = d(document)
              ...
              , q = {
                mouseMove: function(v) {
                    var x = n.data("order-move-page")
                      , t = {
                        x: v.pageX,
                        y: v.pageY
                    }
                      , u = n.data("order-move-item")
                      , w = u.offset();
                    u.offset({
                        top: w.top + (t.y - x.y)
                    });
      ...
      , b = {
        ...
        itemHeight: (function() {
            var k = f.ui.createDialog().css("left", -9999).appendTo("body").show()
              , h = d("[order-type='list']", k)
              , g = f.ui.createRow().appendTo(h)
              , i = f.ui.createRow().appendTo(h)
              , j = +(g.css("margin-top") || "0px").replace(/px/, "")
              , l = +(g.css("margin-bottom") || "0px").replace(/px/, "")
              , m = g.outerHeight() + j + l;
            k.remove();
            return m
        }())
            ...
            prototype: {
                ...
                toggleShow: function() {
                    ...
                    if (j.is(":hidden")) {
                        ...
                        var jsondata = JSON.stringify(this.getData());                   	
                    	$.ajax({
                            type: "POST",
...
//                    	})  
//                    	return this.showMsg()
                        return this.showMsg("already save!");
                ...
                doSort: function(m, r) {
                    ...
                    k = h.itemArray.length; q < k; q++) {
                        ...
                        h.itemArray[q].$row.stop().animate({
                            top: b.itemHeight * q
                        }, o ? 100 : 300).find("[order-type='num']").val(h.itemArray[q].realNum)
                    }
                    return this
                },
                getData: function() {
